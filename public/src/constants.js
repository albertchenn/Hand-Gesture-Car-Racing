export const ANGLE_THRESHOLD = 0.175;

export const VELOCITY_CUSHION = 1;

export const BACKGROUND_SCALE = 20;

export const CAR_PNG = '/Assets/car.png';
export const BACKGROUND_PNG = '/Assets/final-final-race-track.png';

export const VIDEO_WIDTH = 480;
export const VIDEO_HEIGHT = 360;

export const VIDEO_WIDTH_PX = "480px";
export const VIDEO_HEIGHT_PX = "360px";

export const BG_OFFSET_WIDTH = 0;
export const BG_OFFSET_HEIGHT = 0;

export const ON_ROAD_COLOR = { r: 128, g: 128, b: 128 };
export const FINISH_COLOR = { r: 255, g: 0, b: 0 };
export const CHECKPOINT_1_COLOR = { r: 175, g: 0, b: 255 };
export const CHECKPOINT_2_COLOR = { r: 150, g: 0, b: 255 };
export const CHECKPOINT_3_COLOR = { r: 125, g: 0, b: 255 };
export const CHECKPOINT_4_COLOR = { r: 100, g: 0, b: 255 };
export const CHECKPOINT_5_COLOR = { r: 75, g: 0, b: 255 };
export const CHECKPOINT_6_COLOR = { r: 50, g: 0, b: 255 };
export const CHECKPOINT_7_COLOR = { r: 25, g: 0, b: 255 };
export const CHECKPOINT_8_COLOR = { r: 0, g: 0, b: 255 };

export const CHECKPOINTS = [CHECKPOINT_1_COLOR, CHECKPOINT_2_COLOR, CHECKPOINT_3_COLOR, CHECKPOINT_4_COLOR, CHECKPOINT_5_COLOR, CHECKPOINT_6_COLOR, CHECKPOINT_7_COLOR, CHECKPOINT_8_COLOR, FINISH_COLOR];